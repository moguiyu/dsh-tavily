...
0.2.0 将直接提供的 `tavily_search` 模型工具替换为 DSH 原生 `web_search` provider，并新增 `tavily_extract`、`tavily_map` 和 `tavily_crawl`。已有的 `TAVILY_API_KEYS` 凭据和 `tavily-manager.json` 状态会被复用。
