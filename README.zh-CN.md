...
> **要求** — DSH 为开发者预览版（`0.1.x-rc/alpha`）；keyed 插件配置卡片需要 **0.1.0-rc.7 或更新**（含 `0.1.2-alpha.x`、`0.1.3` 版本线，以及截至 `0.1.5-rc.2` 的 `0.1.5` 版本线）。npm 名为作用域包 `@moguiyu/dsh-tavily`，并非同名社区 `dsh-tavily` 提供方替换插件。

> **兼容性** — 同一个包覆盖当前所有 host 版本线：DSH `0.1.0-rc.7` – `0.1.1-rc.x` 使用 rc.7/rc.8 settings 通道；DSH `0.1.2-alpha.2+`（覆盖整个 `0.1.3` 版本线以及当前 `0.1.5` 版本线，含 alpha/RC）使用 `installSection` 通道——运行时自动探测，无需额外安装步骤。peer 范围接受 `^0.1.5-0`，因此 `0.1.5` 的预发布版也算满足。
